...
from luaparser.astnodes import Node, Chunk, Block, Invoke, Name, String, Concat, If, ElseIf, Function, Assign, Number
import glob
...
class LuaInterpreter:
    ...
    def get_json_analyses(self, output_dir: str = './data/rules_parsed'):
        ...
        for file in self.files:
            ...
            conditions_file = os.path.join(file_output_dir, f'{base_name}-conditions.json')
    ...
    # ------------------------------------------------------------------------
    # Resolução de condições + variáveis
    # ------------------------------------------------------------------------
    #
    # Estratégia: durante o percurso da árvore, o caminho de condições de
    # cada ramo (`branch_path`) inclui tanto as condições "positivas"
    # (a condição do if/elseif tomado) quanto negações "NOT (...)" dos
    # ramos anteriores que não foram tomados. Essas negações são
    # necessárias internamente para que `_merge_branch_states` resolva
    # corretamente o valor de uma variável em cada ramo possível — mas
    # não fazem parte do que queremos expor no JSON final. Por isso,
    # toda vez que um caminho de condições é anexado a um objeto de
    # saída (`obj`), ele passa por `_visible_conditions`, que filtra as
    # negações.
    ...
    def _handle_var_node(self, node: dict, base_path: list, var_state: dict, obj: list) -> dict:
    ...
    def _process_if_chain(self, chain: list, base_path: list, var_state_in: dict, obj: list) -> dict:
        """Percorre um bloco if/elseif*/else, retornando o var_state
        resultante da junção de todos os ramos possíveis."""
        branch_results, has_else = self._collect_branch_results(chain, base_path, var_state_in, obj)
    ...
    def _collect_branch_results(self, chain: list, base_path: list, var_state_in: dict, obj: list):
    ...
    def _merge_branch_states(self, chain: list, var_state_in: dict, branch_results: list) -> dict:
    ...
    def _resolve_value(self, val, var_state: dict, base_path: list):
        ...
        if isinstance(val, dict):
            return {k: self._resolve_value(v, var_state, base_path) for k, v in val.items()}
    ...
    def _dedup_entries(self, entries) -> list:
        ...
        for value, path in entries:
            key_value = value if isinstance(value, (str, int, float, bool, type(None))) \
    ...
    # ------------------------------------------------------------------------
    #
    # Varre o resultado (já achatado) de `map_conditions` de cada rule e
    # registra, para cada símbolo (valor de um hit do tipo 'symbol'), quais
    # rules o utilizam. Um mesmo hit pode se resolver em mais de um símbolo
    # possível (quando depende de variável condicional) — nesse caso a rule
    # entra na lista de todos eles.

    SYMBOL_INSTRUCTION_TYPE = 'symbol'
    SYMBOL_KEY = 'PointInstruction'
    ...
    def _collect_symbol_rules(self, conditions: list, rule_name: str, symbol_rules: dict) -> None:
        for item in conditions:
            if item.get('node_type') != 'hit' or item.get('instruction_type') != self.SYMBOL_INSTRUCTION_TYPE:
                continue

            values = item.get('values') or {}
            symbol_value = values.get(self.SYMBOL_KEY)
    ...
    def build_tree(self, tree: Chunk) -> list:
        ...
        if isinstance(tree, Chunk): return self.build_tree(tree.body)
        
        if isinstance(tree, Block): return self.build_tree_body(tree.body)
    ...
    def build_tree_body(self, parent_node: Node):
        ...
        if not parent_node: return nodes_childrens
        
        for node in parent_node: nodes_childrens.extend(self.build_node(node))
    ...
    def build_node(self, node: Node):
        ...
        if isinstance(node, If): return self.build_if_chain(node)
        ...
        if isinstance(node, Block): return self.build_tree_body(node.body)
    ...
    def build_if_chain(self, node: If | ElseIf | Block, branch_type: str = 'if'):
        ...
        if orelse is None: return chain
        
        if isinstance(orelse, ElseIf): chain.extend(self.build_if_chain(orelse, branch_type='elseif'))
...
if __name__ == "__main__":
    intp = LuaInterpreter('source/rules/*.lua')
    intp.get_json_analyses()